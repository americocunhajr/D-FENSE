...
DengueSprint2025_AggregatedData_SC <- read.csv("DengueSprint2025_AggregatedData_SC.csv")
DengueSprint2025_AggregatedData_SC$ln100_casos <- log(DengueSprint2025_AggregatedData_SC$cases+100)
DengueSprint2025_AggregatedData_SC_ts <- ts(DengueSprint2025_AggregatedData_SC,
                                            start = c(2010,1),
                                            frequency = 52)
plot(DengueSprint2025_AggregatedData_SC_ts[,"ln100_casos"])



# Modelo diferença sazonal para o período T1

plot(window(DengueSprint2025_AggregatedData_SC_ts[,"ln100_casos"],
            start=c(2010,52),
            end = c(2021,25)))
# Devido à grande mudança nos anos recentes, diminui a janela de estimação
(m1 <- auto.arima(window(DengueSprint2025_AggregatedData_SC_ts[,"ln100_casos"],
                         start=c(2011,02),
                         end = c(2021,25)),
                  xreg = cbind(window(DengueSprint2025_AggregatedData_SC_ts[,"temp_med"],
                                      start=c(2010,52),
                                end = c(2021,23)),
                               ts(rollsumr(DengueSprint2025_AggregatedData_SC_ts[,"precip_med"],k=52)/52,
                                  start=c(2010,52),
                                  end = c(2021,23),
                                  freq=52))))
...
(M1 <- TSA::arima(window(DengueSprint2025_AggregatedData_SC_ts[,"ln100_casos"],,
                         start=c(2019,02),
                         end = c(2021,25)),
                  order = c(1,0,1)
                  , seasonal = list(order = c(1,1,1))#,io=list(out_m1$ind)
                  ,xreg = cbind(window(DengueSprint2025_AggregatedData_SC_ts[,"temp_med"],
                                       start=c(2018,52),
                                       end = c(2021,23)),
                                ts(rollsumr(DengueSprint2025_AggregatedData_SC_ts[,"precip_med"],k=52)/52,
...
for_M1 <- predict(M1, n.ahead=67
                  ,newxreg=cbind(window(DengueSprint2025_AggregatedData_SC_ts[,"temp_med"],
                                        start=c(2021,24),
                                        end = c(2022,38)),
                                 ts(rollsumr(DengueSprint2025_AggregatedData_SC_ts[,"precip_med"],k=52)/52,
                                    start=c(2021,24),
...
data <- window(DengueSprint2025_AggregatedData_SC_ts[,"epiweek"],
               start=c(2021,26),
               end=c(2022,40))
...
ub_90 <- for_M1$pred+e80*for_M1$se
lb_80 <- for_M1$pred-e80*for_M1$se
ub_80 <- for_M1$pred+e90*for_M1$se
